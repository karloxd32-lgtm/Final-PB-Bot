...
## python-telegram-bot (Async + Timeout Fix)
...
DEFAULT_FORCE_CHANNEL_ID = -1002699957030
DEFAULT_FORCE_BUTTON_NAME = "✅ ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ"

# Optional: Forward uploads to a channel/group (set None to disable)
...
with DB_LOCK:
    # Keep schema same as SQLite (int 0/1 flags) to avoid logic changes.
    cursor.execute("""
...
async def send_start_screen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if photo_id:
        await msg.reply_photo(photo_id, caption=START_CAPTION, reply_markup=keyboard)
    else:
...
async def send_about_screen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if photo_id:
        await msg.reply_photo(photo_id, caption=ABOUT_TEXT, parse_mode="Markdown", reply_markup=kb)
    else:
...
async def set_bot_commands(app):
    commands = [
        ...
        BotCommand("request", "sᴇɴᴅ ʀᴇǫᴜᴇsᴛ"),
...
async def main_async():
    ...
    logger.info("🚀 Bot running (PostgreSQL + Fancy UI + Dynamic Force-Join + /getid + owner-admin)...")
